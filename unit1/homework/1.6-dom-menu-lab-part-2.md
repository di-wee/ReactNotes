...
Select and store the `<nav id="sub-menu">` element in a variable named `subMenuEl`.
...
Select and store all of the `<a>` elements inside of `topMenuEl` in a variable named `topMenuLinks`.
