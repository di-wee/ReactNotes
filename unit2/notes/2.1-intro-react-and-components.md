...
If the create-vite package is not fount you'll get this message
...
The (y) shows the default answer, just `enter` and allow npm to install the package.

After installation, you'll need to input a few bits of data.

You'll need to provide a Project name, we are using `react-app`.
You can ignore the package name as it will use the project name as the package name.

Select `React` for the framework and `JavaScript` for the variant.

The following would be the cli layout.
...
Scaffolding project in /path/to/directory/react-app...
...
You will then need to `cd react-app` and `npm install`.

`npm install` would take time as it installs all the packages required by React.

Your skeleton React app should look like this
<img src="https://i.imgur.com/k7jW5XK.png" height="400" />
and it is ready for development

`npm run dev` is used to start the ReactApp server for you to view the React app on the brower. The server will automatically restart itself whenever you make changes to your React app and save it.

> Although `npm create vite@latest` is good, it is very long-winded, there is a shortcut command
> 
> `npm create vite@latest <app-name> -- --template react`
> 
> this command skips the Q&A part and would just create the scaffolding project. You will still need to `cd <app-name>` and `npm install` to create the skeleton React app

Although using<br>`npm create vite@latest`<br>is a great way to start an actual React project, we can save our disk space when learning and experimenting by using the most excellent code playground, [CodeSandbox](https://codesandbox.io/)...
